import type { ToolPolicy, ToolCall, ToolCallDecision } from "../guard/types.js";

// helpers
function deny(reason: string): ToolCallDecision {
  return { allowed: false, reason };
}
function allow(reason?: string): ToolCallDecision {
...
function stringIncludesAny(s: string, needles: string[]) {
...
export const defaultToolPolicies: Record<string, ToolPolicy> = {
  ...
  "db.query": {
    ...
    validateCall: (call: ToolCall) => {
      ...
      const sql: string | undefined = args.sql ?? args.query;
  ...
  "http.get": {
    ...
    validateCall: (call: ToolCall) => {
      ...
      if (!url || typeof url !== "string") return deny("http.get requires args.url (string)");
  ...
  "file.read": {
    ...
    validateCall: (call: ToolCall) => {
      ...
      if (!path || typeof path !== "string") return deny("file.read requires args.path (string)");

      // prevent path traversal
      if (path.includes("..")) return deny("Path traversal blocked");

      // allow only a safe prefix
      if (!path.startsWith("data/")) return deny("Only data/ directory is readable");
  ...
  "slack.send": {
    validateCall: (call: ToolCall) => {
      const args: any = call.args ?? {};
      if (!hasAnyKey(args, ["channel", "text"])) return deny("slack.send requires channel and text");
      if (typeof args.text !== "string") return deny("slack.send text must be string");
      if (args.text.length > 2000) return deny("Message too long");
      return allow();
    }
  },
  ...
  "email.send": {
    validateCall: (call: ToolCall) => {
      const args: any = call.args ?? {};
      if (!hasAnyKey(args, ["to", "subject", "body"])) return deny("email.send requires to, subject, body");
      if (typeof args.body !== "string") return deny("email body must be string");
      if (args.body.length > 10_000) return deny("Email body too long");
      return allow();
    }
  }
};