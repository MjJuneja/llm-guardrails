...
const RELATION_RE =
  /\bmy (?:son|daughter|child|children|kid|kids|toddler|newborn|infant)\b/gi;

const MINOR_TERM_RE = /\b(?:under[\s-]?18|underage|minors?)\b/gi;

const SCHOOL_RE =
  /\b(?:(?:in\s+)?(?:grade|class|standard)\s+(?:[1-9]|1[0-2])|[1-9](?:st|nd|rd|th)\s+graders?|pre[\s-]?school|kindergarten|nursery school|elementary school|middle school|high school)\b/gi;

// Captures an age in one of three shapes; the numeric check happens in code.
const AGE_RE =
  /\b(?:(\d{1,2})\s*(?:years?|yrs?)\s*old|aged?\s*[:=]?\s*(\d{1,2})\b|(?:i am|i'm)\s+(\d{1,2})\b)/gi;
 ...
 */
export function detectChildSignals(text: string): string[] {
