...
type IndianPIIPattern = {
  type: IndianPIIType;
  re: RegExp;
  replacement: string;
  /** Optional checksum/structural check applied to each regex match. */
...
const INDIAN_PII_PATTERNS: readonly IndianPIIPattern[] = [
  {
    type: "aadhaar",
...
export function detectIndianPII(text: string): IndianPIIDetection {
  const detectedItems: IndianPIIDetectionItem[] = [];
