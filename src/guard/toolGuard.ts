...
export type ToolGuardConfig = {
  ...
  denylist?: string[];  // Blocked tool names
  defaultAction?: ToolValidationAction;
  validators?: ToolValidator[]; // Global validators
  toolRules?: Record<string, ToolValidator[]>; // Tool-specific validators
};
...
export class ToolGuard {
  private globalValidators: ToolValidator[] = [];
  private toolSpecificRules: Map<string, ToolValidator[]> = new Map();
  private allowlist: Set<string> | null = null;
  private denylist: Set<string> | null = null;
  private defaultAction: ToolValidationAction = "allow";
   ...
   */
  async validate(call: ToolCall): Promise<ToolValidationResult> {
    const { name } = call;
    ...
    const activeValidators: ToolValidator[] = [...this.globalValidators];
    ...
    const results = await Promise.all(
      activeValidators.map(async (v) => {
        try {
          ...
          return {
            action: "block" as ToolValidationAction,
            riskScore: 100,
    ...
    // 4. Aggregate results
    let finalAction: ToolValidationAction = "allow";
    let maxRiskScore = 0;
    const matchedRules: string[] = [];
    const reasons: string[] = [];
    const triggeredValidators: string[] = [];
...
export function isPathInside(filePath: string, rootDir: string): boolean {
...
function parseUrlSafe(val: string): URL | null {
  try {
    let urlStr = val.trim();
...
function findArgRecursive(obj: any, keys: string[]): { key: string; value: any } | null {
...
export type FilesystemValidatorConfig = {
  ...
  argNames?: string[];
  actionOnViolation?: ToolValidationAction; // default block
  riskScoreOnViolation?: number; // default 90
};

export function createFilesystemValidator(config: FilesystemValidatorConfig = {}): ToolValidator {
...
export type SqlValidatorConfig = {
  ...
  allowlist?: string[]; // Allowed SELECT/WITH queries regex strings
  argNames?: string[];
  actionOnViolation?: ToolValidationAction; // default block
  riskScoreOnViolation?: number; // default 95
};

export function createSqlValidator(config: SqlValidatorConfig = {}): ToolValidator {
...
export type ShellValidatorConfig = {
  ...
  argNames?: string[];
  actionOnViolation?: ToolValidationAction; // default block
  riskScoreOnViolation?: number; // default 98
};

export function createShellValidator(config: ShellValidatorConfig = {}): ToolValidator {
...
export type HttpValidatorConfig = {
  allowlist?: string[]; // allowed domain glob patterns (e.g. *.example.com)
  denylist?: string[];  // blocked domain glob patterns
...
export function createHttpValidator(config: HttpValidatorConfig = {}): ToolValidator {
  const allowlist = config.allowlist || [];
  ...
  function matchesPatterns(host: string, patterns: string[]): boolean {
...
export type FinancialValidatorConfig = {
  maxAmount: number;
  currency?: string; // e.g. "USD" (case-insensitive check)
...
export function createFinancialValidator(config: FinancialValidatorConfig): ToolValidator {
...
export function createCustomValidator(name: string, fn: CustomValidatorFn): ToolValidator {
  return {
    name,
