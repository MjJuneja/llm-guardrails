...
function utf8Bytes(input: string): number[] {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    } else if (cp >= 0xd800 && cp < 0xdc00 && i + 1 < input.length) {
      const cp2 = input.charCodeAt(i + 1);
...
export function hashMatch(s: string): string {
  return sha256Hex(s).slice(0, 16);
}

export function maybeHash(matches: string[], enabled: boolean): string[] {
